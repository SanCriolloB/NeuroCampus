...
prep-validate:
	...
	$(PYTHON) -c "import glob,pyarrow.parquet as pq,sys; files=sorted(glob.glob('$(PREP_DIR)/*.parquet')); sys.exit(1) if not files else None; ok=True; [ (lambda cols: (print(p,'feat_t_*:', any(c.startswith(\"feat_t_\") for c in cols), 'label:', any(c in cols for c in ('y_sent','sentimiento','label_sent')), 'n_cols:', len(cols)), globals().__setitem__('ok', ok and any(c.startswith('feat_t_') for c in cols) and any(c in cols for c in ('y_sent','sentimiento','label_sent')))) )(pq.read_table(p).column_names) for p in files ]; sys.exit(0 if ok else 1)"
